...
// for at least the counter duration
  ...
  wait_us(0.1);  // Add delay for accumulation of samples
...
{  
 ...
 // wake up device
  ...
  wait_us(0.1); // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt  
...
// Configure FIFO to capture accelerometer and gyro data for bias calculation
  ...
  wait_us(0.08); // accumulate 80 samples in 80 milliseconds = 960 bytes
...
void MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
   ...
   wait_us(0.25);  // Delay a while to let the device execute the self-test
